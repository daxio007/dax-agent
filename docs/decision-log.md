...
## 2026-06-17：先设计 Agent Core 第一阶段实现方案

决策：

在继续实现大脑代码之前，先新增 `docs/agent-core-implementation-plan.md`，把 Agent Core 第一阶段的类型、模块、方法、JSDoc、审计、API、验证计划和接入顺序固定下来。

原因：

读、听、说、手、脚第一阶段都已经成为当前基线。Agent Core 将会控制这些能力，尤其会影响手和脚这种会产生真实副作用的能力。如果直接写代码，容易把“大脑”写成一次模型调用，或者让模型绕过 Policy Gate 直接写入、执行。先写实现设计，可以确保后续代码仍然坚持“模型给候选，代码管骨架和边界”。

影响：

- 新增 `docs/agent-core-implementation-plan.md`。
- 明确第一阶段主流程为 `ListenResult -> Agent Core -> AgentDecision -> CapabilityRoute -> SpeakPlan/ActionProposal`。
- 明确新增 `src/lib/core.ts`，并在 `src/lib/types.ts`、`src/lib/store.ts`、`src/server.ts` 和 `src/lib/agent.ts` 中接入 Agent Core。
- 明确手和脚第一阶段只能通过 `ActionProposal` 进入大脑输出，不能被 Agent Core 直接自动执行。
- 明确每个 exported 方法和关键 helper 都必须写详细 JSDoc，说明使用方法、作用和边界。

## 2026-06-18：实现 Agent Core 第一阶段运行时

决策：

根据 `docs/agent-core-implementation-plan.md` 实现最小大脑运行时，并把自然语言消息主流程改为 `listen -> Agent Core -> optional read -> Agent Core -> speak`。模型只生成结构化候选，代码负责 hard control、schema 校验、fallback、Policy Gate、CapabilityRoute 和 audit。

原因：

读、听、说、手和脚已经具备独立能力核心，但自然语言消息仍然是听完后直接调用模型，缺少统一决策层。没有 Agent Core 时，模型工具请求容易把“思考建议”和“真实执行”混在一起，也无法统一处理暂停、上下文读取、记忆候选和手脚行动边界。

影响：

- 新增 `src/lib/core.ts` 和 Agent Core 类型。
- 新增 Agent Core store、audit 和 HTTP API。
- `processUserMessage()` 已接入一次受限 read round 和二次决策。
- echo、模型错误、非法 JSON 和候选校验失败都会进入本地 fallback。
- 模型普通文本不再直接创建 `tool_request`；手和脚只生成 `ActionProposal`。
- Policy Gate 明确阻止 `hand.apply`、`foot.execute`、敏感原文记忆和未实现的 Skill Runtime。
- 新增 `docs/agent-core-implementation.md`。
- 所有新增 exported 方法和关键 helper 都有详细 JSDoc，说明使用方法、作用和边界。

## 2026-06-18：明确模型密钥保存状态并增加连接测试

决策：

设置 API 继续只返回脱敏后的 API key，不把明文密钥重新发送到浏览器；设置面板新增“已有密钥”状态、Provider 说明、配置校验、保存反馈和连接测试。

原因：

密码框为了安全会在每次打开时保持空白，但旧界面没有显示已有密钥状态，容易让用户误以为保存失败。同时 Base URL、模型和 API key 已填写时，如果 Provider 仍是 `echo`，运行时仍只会返回本地演示内容，看起来像设置不生效。

影响：

- Echo 模式会禁用外部模型字段并明确说明不会调用 API。
- OpenAI-compatible 设置会校验 Base URL、模型名称和已有或新输入的 API key。
- 保存成功后设置对话框会显示当前有效 Provider。
- API key 继续不回显明文，只显示脱敏后的已保存状态。
- 新增 `POST /api/config/test`，用于发出最小模型连接测试。
- 用户当前模型配置已切换为 OpenAI-compatible，并已验证连接成功。

## 2026-06-22：全量补齐并自动审计 JSDoc

决策：

JSDoc 规范从“新能力的 exported 方法和关键 helper”扩大为全部 `src/**/*.ts` 命名方法。每个方法都必须明确说明使用方法和作用；涉及路径、安全、持久化、外部请求、模型或执行副作用时，还应说明边界。

原因：

项目已有部分通用模块、服务端路由 helper 和前端方法没有方法级文档，与用户持续要求的“代码必须清晰明了”不一致。只依靠开发时提醒无法避免后续遗漏，需要把规范变成可执行检查。

影响：

- 全量扫描 TypeScript 源码并补齐 80 个缺失的 JSDoc。
- 新增 `docs/jsdoc-standard.md`，统一记录覆盖范围、内容要求和示例。
- 新增 `scripts/check-jsdoc.mjs`，使用 TypeScript AST 检查函数、方法、构造器、访问器和命名箭头函数。
- 新增 `npm run check:jsdoc`。
- 检查要求每段受约束的 JSDoc 至少包含“使用方法”和“作用”。
- 2026-06-22 进一步要求方法签名中的每个参数都必须有带用途说明的 `@param`。
- 全量为 345 个带参数的方法补充 544 个 `@param` 标签。
- 自动检查同时验证参数名称和说明文字，空标签或遗漏参数都会失败。
- 匿名内联回调不强制单独写 JSDoc；复杂回调应提取为命名方法。
- 后续 TypeScript 源码改动必须同时通过 JSDoc 检查、typecheck 和 build。

## 2026-06-22：代码修改完成后必须创建 Git 提交

决策：

把 Git 提交纳入每次代码任务的强制收尾流程。完成代码修改、相关文档更新和必要验证后，Codex 必须将本次任务相关改动提交到当前分支。

原因：

用户希望仓库始终保留清晰、可追踪的开发节点，不希望已经完成的代码长期停留在未提交工作区，也不希望每次都需要额外提醒提交。

影响：

- 新增仓库根目录 `AGENTS.md`，让未来代码代理会话可以直接读取该约束。
- 标准流程变为：设计或确认范围 -> 修改代码和文档 -> 运行检查 -> 核对 diff -> 创建 Git 提交。
- 提交应包含本次任务相关的代码、检查脚本、测试和文档。
- 不得为了提交而包含用户的无关修改。
- 创建提交不代表推送远端；`git push` 仍需要用户明确要求。
